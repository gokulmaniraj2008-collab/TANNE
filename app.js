  ...
  try {
    ...
    onValue(ref(db, 'config/menu'), snap => {
      if (snap.exists() && snap.val() && snap.val().length > 0) {
        ...
        // No menu in Firebase — save DEFAULT_MENU automatically
...
window.doSignup = function() {
  ...
  get(ref(db, `accounts/${safeEmail}`)).then(snap => {
    if (snap.exists()) return showError('signupError', 'An account with this email already exists.');
    // Store hashed password using btoa (basic obfuscation — upgrade to Firebase Auth for production)
    set(ref(db, `accounts/${safeEmail}`), { name, email, pass: btoa(pass) });
...
window.doLogin = function() {
  ...
  const safeEmail = email.replace(/\./g, '_').replace(/@/g, '_at_');
  get(ref(db, `accounts/${safeEmail}`)).then(snap => {
    if (!snap.exists()) return showError('loginError', 'Incorrect email or password.');
    const account = snap.val();
    if (btoa(pass) !== account.pass) return showError('loginError', 'Incorrect email or password.');
...
window.savePassword = async function() {
  ...
  if (!snap.exists() || snap.val().pass !== btoa(current))
    return showError('changePassError', 'Current password is incorrect.');

  await set(ref(db, `accounts/${currentUserId}/pass`), btoa(next));
