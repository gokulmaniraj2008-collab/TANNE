...
function renderMenu(query) {
  ...
  MENU.forEach(cat => {
    ...
    filtered.forEach(item => {
      ...
      const display  = q
        ? (function() {
            // Split on all matches (case-insensitive) and highlight each one
            const escapedQ = q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const parts = item.name.split(new RegExp(`(${escapedQ})`, 'gi'));
            return parts.map(p =>
              p.toLowerCase() === q.toLowerCase()
                ? `<span style="color:var(--gold);font-weight:700">${sanitize(p)}</span>`
                : sanitize(p)
            ).join('');
          })()
        : sanitize(item.name);
